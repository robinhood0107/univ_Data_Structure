//소스 코드 5.7:Binary Search Tree
/* Binary Search Tree
- 화면에서 입력받아 BST를 만든다
- 입력후 left subtree의 노드 수 – right subtree의 노드 수 >= 5 이상이면 split 호출
- adjust하는 프로그램 작성-balanced tree를 만드는 방법
*/
#include <iostream>
using namespace std;
...
public:
	...
	//bool Delete(const Element&);//구현 실습, 특히 non-leaf node의 삭제시에 inorder successor( )를 구해 replace하는 것이 필요
	BstNode* Search(BstNode*, const Element&);//private function으로 이동해야 한다.
...
private:
	BstNode* root;
	bool DeleteLargestElement(Element& outElement);//TwoWayJoin( )에서 호출된다
};
...
{
    BstNode* p = root;
    BstNode* q = 0; // [수정] 여기에 q를 선언해야 합니다.
...
int main() {
	...
	Element* tempEle;
	do {
		cout << "binary search tree. Select: 1 insert, 2 display, 3 split and join, >=5 exit" << endl;//delete를 추가하는 구현 실습
		cin >> select;
		switch (select) {
		case 1:
			cout << "input value: ";
