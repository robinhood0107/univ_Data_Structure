...
public:
    ...
    void Insert(vector<int>& vec) {
        for (int x : vec) { // 벡터의 모든 원소를 순회하며
            Insert(x);      // 위의 단일 Insert 함수 재사용
        }
    }
...
int main() {
    ...
    // 1. 벡터 선언 및 난수 100개 채우기
    vector<int> vec; 
    cout << "1. Generate 100 Randoms into Vector" << endl;
    
    for(int i=0; i<100; i++) {
        vec.push_back(rand() % 1000); // 벡터에 값 추가
    }
    // 2. 벡터 통째로 넘기기
    tree.Insert(vec);
